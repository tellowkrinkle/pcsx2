...
static void rpsxLB()
{
	...
	xFastCall((void*)iopMemRead8, ecxd );		// returns value in EAX
	if (_Rt_) {
		xMOVSX(eaxd, al);
...
static void rpsxLH()
{
	...
	xMOV(ecxd, ptr32[&psxRegs.GPR.r[_Rs_]]);
	if (_Imm_) xADD(ecxd, _Imm_);
	xFastCall((void*)iopMemRead16, ecxd );		// returns value in EAX
	if (_Rt_) {
		xMOVSX(eaxd, ax);
		xMOV(ptr32[&psxRegs.GPR.r[_Rt_]], eaxd);
	}
...
static void rpsxLW()
{
	...
	xFastCall((void*)iopMemRead32, ecxd );		// returns value in EAX
	if (_Rt_) {
		xMOV(ptr32[&psxRegs.GPR.r[_Rt_]], eaxd);
	}
	j8Ptr[1] = JMP8(0);
	...
	xMOV(ecxd, ptr32[xComplexAddress(rax, iopMem->Main, rcx)]);
	if (_Rt_) {

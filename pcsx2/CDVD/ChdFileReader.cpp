...
bool ChdFileReader::Open2(const wxString& fileName)
{
	...
	while (CHDERR_REQUIRES_PARENT == (error = chd_open(chds[chd_depth].c_str(), CHD_OPEN_READ, NULL, &child)))
	{
		...
		{
			Console.Error(L"chd_open hit recursion limit searching for parents");
			return false;
		}
		if (chd_read_header(chds[chd_depth].c_str(), &header) != CHDERR_NONE)
		{
			Console.Error(L"chd_open chd_read_header error: %s: %s", chd_error_string(error), WX_STR(chds[chd_depth]));
			return false;
		}
		...
		if (!found_parent)
		{
			Console.Error(L"chd_open no parent for: %s", WX_STR(chds[chd_depth]));
			break;
		}
	}
	...
	{
		Console.Error(L"chd_open chd_read_header error: %s: %s", chd_error_string(error), WX_STR(chds[0]));
		return false;
	}
